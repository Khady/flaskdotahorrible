...
def post_hero(name=None):
    if (request.method == 'POST'):
        ...
        cur = g.db.execute('select id from hero where nam like ?', [request.form['nam']])
        ...
        if len(entries) == 0:
            ...
            g.db.execute('update hero set typ = ?, des = ?, bio = ?, str_start = ?, agi_start = ?, int_start = ?, str_lvl = ?, agi_lvl = ?, int_lvl = ?, life = ?, mana = ?, damages = ?, range = ?, cast_speed = ?, anim_speed = ?, vision = ?, armor = ?, aspeed = ?, ms = ?, nam = ? where nam like ?',
                         [request.form['typ'], request.form['des'],
                          ...
                          request.form['str_lvl'], request.form['agi_lvl'],
                          request.form['int_lvl'], request.form['life'],
                          request.form['mana'], request.form['damages'],
                          request.form['range'], request.form['cast_speed'],
                          request.form['anim_speed'], request.form['vision'],
                          request.form['armor'], request.form['aspeed'],
                          request.form['ms'], request.form['nam'], request.form['nam']])
        g.db.commit()
        g.db.close()
        ...
        if name == None:
            ...
            entries = [dict(id_hero=row[0], nam=row[1], typ=row[2], des=row[3], bio=row[4], 
            ...
            if (len(entries) == 0):
                ...
                return render_template('post_hero.html', entries = entries)
