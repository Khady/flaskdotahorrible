    ...
    if (request.method == 'POST'):
        if request.form['mode_post'] == 'Previsualisation':
            ...
            entries = dict(titre=request.form['titre'],
                           news=request.form['news'], tag=request.form['tag'])
            return render_template('post_news.html', entries=entries, prevu=1,
        ...
        if (id_news == None):
            ...
            entries = [dict(titre=row[1], autor=row[2],
                            tag=row[3], news=row[4]) for row in cur.fetchall()]
            if len(entries) != 0:
                return render_template('post_news.html', entries=entries[0],
                                       id_news=id_news)
            else:
                return render_template('post_news.html')
